<template>
          ...
          <div class="cards" :style="{ transform: `translateX(-${currentIndex * (100 / slidesToShow)}%)` }">
            ...
            <div v-else class="card" v-for="noticia in noticias" :key="noticia.id" @click="irADetalle(noticia.id)">
              ...
              <div class="card-content">
                ...
                <p>{{ recortarContenido(noticia.contenido) }}</p>
                <span>Fecha de Publicación: {{ noticia.fechaPublicacion }}</span>
                <span v-if="noticia.estado">{{ noticia.estado }}</span>
              </div>
            </div>
          </div>
  ...
  const fetchNoticias = async () => {
    try {
      ...
      if (data && Array.isArray(data.body)) {
        ...
        console.warn('No se encontraron noticias o la estructura de datos no es válida.');
      }
    } catch (error) {
  ...
  const nextSlide = () => {
    if (currentIndex.value < Math.ceil(noticias.value.length / slidesToShow.value) - 1) {
      currentIndex.value++;
    }
  };
  
  // Función para navegar a la vista de detalle de la noticia
  const irADetalle = (id) => {
    console.log('ID de la noticia al hacer clic:', id); // Verificar que el ID esté disponible
    if (id) {
      router.push(`/noticia/${id}`); // Navega a la vista de detalle con el ID
    } else {
